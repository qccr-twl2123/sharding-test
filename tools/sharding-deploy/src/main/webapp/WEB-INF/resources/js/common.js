...
//非空校验
function isNull(){
	$("body").on('focus',"input.required:not(:hidden),select.required:not(:hidden),textarea.required:not(:hidden)",function(){
	...
	var isNullSub=false;
	if(!isNullSub && $(".requiredMust").length>0){
		$(".requiredMust").each(function(){
			if(!$(this).val()){
				alert($(this).data('msg'));
				isNullSub=true;
				return false;
			...
			return this.each(function() {
				...
				UE.getEditor($element.attr("id"), options).ready(function() {
					this.execCommand("serverparam", {
						...
						project:(options==null||options.project==null)?"shop":options.project
					});
				});
			});
		}
